...
const bumpType =
  args.find((a) => a.startsWith("--bump-type="))?.split("=")[1] || "patch";
...
// Auto commit
const versions = Object.entries(newVersions)
  .map(([k, v]) => `${k}@${v}`)
  .join(", ");
execSync(
  `git add packages/*/package.json && git commit -m "chore: bump to ${versions}"`,
  { stdio: "inherit" },
);
...
// Create git tag
execSync(`git tag -a v${mainVersion} -m "Release v${mainVersion}"`, {
  stdio: "inherit",
});
